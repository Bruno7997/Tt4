...
function draw() {
  ...
  if(gameState==1&&game.r<qdm){for(var a=0;a<(qdm.toFixed(0));a++){game.r=a
...
for (var i=0;i<m.length;i++){
  ...
  //Velocidade
  if(Math.sqrt((Math.pow(mpx[1],2)+Math.pow(mpy[1],2)))>Math.sqrt((Math.pow(mpx[2],2)+Math.pow(mpy[2],2)))){m[i].speed(p[m[i].indexXY].position.x,p[m[i].indexXY].position.y,i)}
  else{m[i].speed(p[player.index-1].position.x,p[player.index-1].position.y,i)}
  ...
  dy[0]=dy[indexY]
    ...
    if(m.length>0){
    if(m[i].indexXY==player.index){m[i].indexXY+=1}
    else{m[i].indexXY+=1}  
...
//Destruir balas
if(i<buls.lenght&&(buls[i].sprite.position.x>p[player.index-1].position.x+windowWidth/1.9 || buls[i].sprite.position.x<p[player.index-1].position.x-windowWidth/1.9 || buls[i].sprite.position.y>p[player.index-1].position.y+windowHeight/1.9 || buls[i].sprite.position.y<p[player.index-1].position.y-windowHeight/1.9)){buls[i].Destroy(i)}
}
}
//gameState = 0/-1 => faça:
...
function keyPressed() {
  ...
  if(keyCode==27){
    ...
    ref.remove()
    var ref = database.ref("round")
    ref.remove()
    var ref = database.ref("qdm")
    ref.remove()
    form.res();player.res()
...
if(m.length==0&&keyCode==32&&gameState==1){qdm+=(qdm/(Round*0.95));game.update(0)}
  ...
  //W:
if(keyCode===87&&X!=1){setTimeout(()=>{if(player.index!=null){var bul = new Bullet(p[player.index-1].position.x,p[player.index-1].position.y,0,-vdb,d);buls.push(bul)}},500)}
  //S:
if(keyCode===83&&X!=1){setTimeout(()=>{if(player.index!=null){var bul = new Bullet(p[player.index-1].position.x,p[player.index-1].position.y,0,vdb,d);buls.push(bul)}},500)}
  //A:
if(keyCode===65&&X!=1){setTimeout(()=>{if(player.index!=null){var bul = new Bullet(p[player.index-1].position.x,p[player.index-1].position.y,-vdb,0,d);buls.push(bul)}},500)}
  //D:
if(keyCode===68&&X!=1){setTimeout(()=>{if(player.index!=null){var bul = new Bullet(p[player.index-1].position.x,p[player.index-1].position.y,vdb,0,d);buls.push(bul)}},500)}

//Contrario do Relogio
//W:
if(keyCode===87&&X==1){setTimeout(()=>{if(player.index!=null){var bul = new Bullet(p[player.index-1].position.x,p[player.index-1].position.y,-vdb,-vdb,d);buls.push(bul)}},500)}
  //S:
if(keyCode===83&&X==1){setTimeout(()=>{if(player.index!=null){var bul = new Bullet(p[player.index-1].position.x,p[player.index-1].position.y,vdb,vdb,d);buls.push(bul)}},500)}
  //A:
if(keyCode===65&&X==1){setTimeout(()=>{if(player.index!=null){var bul = new Bullet(p[player.index-1].position.x,p[player.index-1].position.y,-vdb,vdb,d);buls.push(bul)}},500)}
  //D:
if(keyCode===68&&X==1){setTimeout(()=>{if(player.index!=null){var bul = new Bullet(p[player.index-1].position.x,p[player.index-1].position.y,vdb,-vdb,d);buls.push(bul)}},500)}
}
